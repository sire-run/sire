...
- **Mechanism:** It uses an `InProcessServer` that wraps the core Go tools (like `file`, `http`, `transform`). This server is **not a network server**; it executes tools via direct function calls within the same process.
- **Flow (`sire:local/file.write`):**
    1.  The `DispatcherMux` routes the call to the `LocalDispatcher`.
    2.  The `LocalDispatcher` calls the `InProcessServer`.
    3.  The `InProcessServer` uses a `NodeRegistry` to instantiate and execute the `file.write` tool.
...
## 3. Durable Execution and State Persistence
...
### 3.2. Stateful Core Data Structures
...
### 3.3. Stateful Engine and Resumption Logic
...
#### 4.1.1. Concurrent and Parallel Execution
