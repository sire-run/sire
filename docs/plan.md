...
*   [ ] T6.1 Implement an in-process MCP server and client  Owner: TBD  Est: 4h
    *   [ ] S6.1.1 Create a singleton `InProcessServer` that can host MCP services without a network listener.
    *   [ ] S6.1.2 Create a `sire:local` dispatcher that acts as a client to this in-process server.
    *   [ ] S6.1.3 Add unit tests for the in-process client-server communication.
...
*   [ ] T9.3 Implement retry and resumption logic  Owner: TBD  Est: 5h
    *   [ ] S9.3.1 When a tool call fails with a transient error, the engine marks the step as `retrying` in the database.
